...
| Exceptions | Typed error values for `errors.Is/As` | `ErrUnsupportedCapability`, `ErrProtectedPort` sentinels; `*WriteVerificationError{Before, After}`, `*CredentialError`, `*ConfigError`, `*HTTPError`, `*HTTPAuthError`, `*SNMPError`, `*NSDPError` types; `NotImplementedError` analogue is a distinct sentinel `ErrKnownUnimplemented`. |
